...
export async function nextVideo(state: State, storage: IStorage, {
  finish,
  reset,
  video,
  playlistKey,
  ...
  const playlistState = await storage.readJSON<PlaylistState>('playlist.json', {});
  ...
  if (update) {
    await storage.writeJSON('state.json', state);
  }
